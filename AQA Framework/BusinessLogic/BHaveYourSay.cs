...
{
    class BHaveYourSay : BaseClass
    {
        IWebDriver driver;
        public BHaveYourSay(IWebDriver driver)
        {
            this.driver = driver;
        }
        ...
        public void FillTheForm(Dictionary<string, string> values, bool screenShot)
        {
            ...
            if (screenShot == true)
                ((ITakesScreenshot)driver).GetScreenshot().SaveAsFile("D:/EPAMCourse/hello.png");
            else
            {
                haveyoursay.submitBtn.Click();
                foreach (KeyValuePair<string, string> value in values)
                {
                    if(value.Value == "" && value.Key == "Name")
                    {
                        if (!haveyoursay.inputErrorNameText.Displayed)
                            throw new Exception();
                    }
                    if (value.Value == "" && value.Key == "Email address")
                    {
                        if (!haveyoursay.inputErrorEmailText.Displayed)
                            throw new Exception();
                    }
                }
            }
